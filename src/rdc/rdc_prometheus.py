...
class PrometheusReader(RdcReader):
    def __init__(self, rdc_ip_port, field_ids, update_freq, max_keep_age, max_keep_samples,
        ...
        for fid in self.field_ids:
            ...
            if any(field_name == f"xgmi_{i}_read" or field_name == f"xgmi_{i}_write" for i in range(8)):
                self.guages[fid] = Gauge(field_name, field_name, labelnames=['gpu_index', "xgmi_label"])
            else:
                self.guages[fid] = Gauge(field_name, field_name, labelnames=['gpu_index'])

    def handle_field(self, gpu_index, value, xgmi_label=None):
        gpu_label = gpu_index
        if self.enable_pci_id:
            gpu_label = self.index_to_bus_addr[gpu_index]
        if value.field_id.value in self.guages:
            guage = self.guages[value.field_id.value]
            if 'xgmi_label' in guage._labelnames:
                #self.guages[value.field_id.value].labels(gpu_label).set(value.value.l_int)
                guage.labels(gpu_label, xgmi_label).set(value.value.l_int)
            else:
                guage.labels(gpu_label).set(value.value.l_int)
